...
class Solution {
    func leastInterval(_ tasks: [Character], _ n: Int) -> Int {
        var taskCountMap: [Character: Int] = [:]
        for tsk in tasks {
            taskCountMap[tsk, default: 0] += 1
        }
        var maxHeap: Heap<Int> = Heap { l, r in
            l > r
        }
        for (_, cnt) in taskCountMap {
        ...
        while maxHeap.isNotEmpty {
            var refill: [Int] = []
            var intrvl: Int = 0
            while intrvl < n + 1 && maxHeap.isNotEmpty {
                let cnt = maxHeap.popTop()!
                if cnt > 1 {
                    refill.append(cnt - 1)
                }
                intrvl += 1
            ...
            res += maxHeap.isEmpty ? intrvl : n + 1
        }
        return res
    }
}
