...
class Solution {
    struct Point {
        let x: Int
        let y: Int
        let distFromZero: Double
        init(x: Int, y: Int) {
            self.x = x
            self.y = y
            self.distFromZero = sqrt(Double(x * x) + Double(y * y))
        }
    }

    func kClosest(_ points: [[Int]], _ k: Int) -> [[Int]] {
        guard k <= points.count else { return [] }
        var minHeap: Heap<Point> = Heap { l, r in
            l.distFromZero < r.distFromZero
        }
        for pt in points {
            minHeap.insert(item: Point(x: pt[0], y: pt[1]))
        }
        var res: [[Int]] = []
        for _ in 0..<k {
            let pt = minHeap.popTop()!
