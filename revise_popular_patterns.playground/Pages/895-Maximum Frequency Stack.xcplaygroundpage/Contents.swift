...
class FreqStack {
    ...
    var maxHeap: Heap<FreqItem>
    var freqMap: [Int: Int]

    init() {
        maxHeap = Heap(comparator: { l, r in
            l.freq > r.freq
        })
        freqMap = [:]
    }
    ...
    func push(_ val: Int) {
        ...
        maxHeap.insert(item: FreqItem(val: val, freq: freqMap[val]!))
    }

    func pop() -> Int {
        let item = maxHeap.popTop()!
        freqMap[item.val, default: 0] -= 1
        return item.val
    }
}
